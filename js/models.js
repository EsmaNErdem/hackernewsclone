...
class StoryList {
  ...
  async addStory(user, {author, title, url}) {
    ...
    let createdStory = response.data.story;
  ...
  /*Deletes stories from API and user data */

  async removeUserStories(user, storyId) {
    let token = user.loginToken;
    const response = await axios({
...
/* Adds favorites to User data and post/removing fav data to/from API */

async addToFavorites(story, storyId) {
